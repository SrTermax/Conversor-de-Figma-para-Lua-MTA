...
function toColor(color, opacity = 1) {
    if (!color) {
...
export function generateLuaCode(nodes, config, imageNames) {
    const drawCalls = [];
    ...
    for (const node of nodes) {
        // Pular Background
        if (node.name.toLowerCase() === config.backgroundName.toLowerCase() ||
    ...
    luaLines.push('-- Conversor de Figma para Lua MTA', '-- Gerado automaticamente pelo plugin Figma Convert Lua', '-- Se o seu projeto tiver imagens, certifique-se de que elas estão baixadas na pasta assets e exportadas no meta.xml!', '-- ECLIPSE não está disponivel nessa versão então evite usar, Vectory pode vim todo preto, ajuste conforme necessário!', '', '-- Agradecimento especial aos lendários: SiiLVa & Baron_Scr - Vocês são brabos demais!', '-- Versão 2025 - Gerado automaticamente pelo Figma Convert Lua. | Versão BETA Pode ocorrer bugs em alguns projetos!', '', '');
    // Variáveis principais
    luaLines.push('local sW, sH = guiGetScreenSize()', 'local isOpen = true', `local resW, resH = ${config.resW}, ${config.resH}`, 'local x, y = sW/resW, sH/resH', '');
    ...
    luaLines.push('end', '');
    // Event handler
    luaLines.push('if isOpen then', '\taddEventHandler("onClientPreRender", root, onClientRender_FigmaConvertMTA)', 'end');
    // Adicionar função de retângulo arredondado se necessário
    if (requiresRounded) {
        luaLines.push('', 'function dxDrawRoundedRectangle(x, y, w, h, color, radius)', '\tw = w - radius * 2', '\th = h - radius * 2', '\tx = x + radius', '\ty = y + radius', '\tif (w >= 0) and (h >= 0) then', '\t\tdxDrawRectangle(x, y, w, h, color)', '\t\tdxDrawRectangle(x, y - radius, w, radius, color)', '\t\tdxDrawRectangle(x, y + h, w, radius, color)', '\t\tdxDrawRectangle(x - radius, y, radius, h, color)', '\t\tdxDrawRectangle(x + w, y, radius, h, color)', '\t\tdxDrawCircle(x, y, radius, 180, 270, color, color, 7)', '\t\tdxDrawCircle(x + w, y, radius, 270, 360, color, color, 7)', '\t\tdxDrawCircle(x + w, y + h, radius, 0, 90, color, color, 7)', '\t\tdxDrawCircle(x, y + h, radius, 90, 180, color, color, 7)', '\tend', 'end');
    }
    return luaLines.join('\n');
}
// Função para gerar meta.xml
export function generateMetaXML(imageNames) {
    const fileEntries = imageNames.map((name) => `  <file src="assets/images/${name}" />`);
